...
CREATE TABLE IF NOT EXISTS registered_addresses (id INTEGER PRIMARY KEY AUTOINCREMENT,address TEXT NOT NULL UNIQUE,display_name TEXT,enabled INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL,updated_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS mail_folders (id TEXT PRIMARY KEY,parent_folder_id TEXT,display_name TEXT NOT NULL,well_known_name TEXT,child_folder_count INTEGER,total_item_count INTEGER,unread_item_count INTEGER,is_hidden INTEGER NOT NULL DEFAULT 0,enabled INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL,updated_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS mail_messages (row_id INTEGER PRIMARY KEY AUTOINCREMENT,id TEXT NOT NULL UNIQUE,internet_message_id TEXT,conversation_id TEXT,conversation_index TEXT,folder_id TEXT NOT NULL,subject TEXT,body_html TEXT,body_text TEXT,body_preview TEXT,body_content_type TEXT,sender_address TEXT,sender_name TEXT,from_address TEXT,from_name TEXT,received_at TEXT,sent_at TEXT,created_at TEXT,modified_at TEXT,deleted_at TEXT,importance TEXT,is_read INTEGER,is_draft INTEGER,has_attachments INTEGER,flag_status TEXT,web_url TEXT,etag TEXT,raw_json TEXT NOT NULL,indexed_at TEXT,FOREIGN KEY(folder_id) REFERENCES mail_folders(id));
CREATE TABLE IF NOT EXISTS mail_recipients (message_row_id INTEGER NOT NULL,recipient_type TEXT NOT NULL,address TEXT NOT NULL,display_name TEXT,normalized_address TEXT NOT NULL,FOREIGN KEY(message_row_id) REFERENCES mail_messages(row_id));
CREATE TABLE IF NOT EXISTS mail_message_addresses (message_row_id INTEGER NOT NULL,registered_address_id INTEGER NOT NULL,matched_by TEXT NOT NULL,matched_value TEXT,PRIMARY KEY(message_row_id,registered_address_id,matched_by),FOREIGN KEY(message_row_id) REFERENCES mail_messages(row_id),FOREIGN KEY(registered_address_id) REFERENCES registered_addresses(id));
...
CREATE TABLE IF NOT EXISTS mail_attachments (id TEXT NOT NULL,message_row_id INTEGER NOT NULL,name TEXT,content_type TEXT,size INTEGER,is_inline INTEGER NOT NULL DEFAULT 0,content_id TEXT,attachment_type TEXT,raw_json TEXT,PRIMARY KEY(message_row_id,id),FOREIGN KEY(message_row_id) REFERENCES mail_messages(row_id));
CREATE TABLE IF NOT EXISTS mail_categories (message_row_id INTEGER NOT NULL,category TEXT NOT NULL,PRIMARY KEY(message_row_id,category),FOREIGN KEY(message_row_id) REFERENCES mail_messages(row_id));
CREATE TABLE IF NOT EXISTS mail_sync_states (folder_id TEXT PRIMARY KEY,next_link TEXT,delta_link TEXT,last_attempt_at TEXT,last_success_at TEXT,last_error TEXT,consecutive_failures INTEGER NOT NULL DEFAULT 0);
CREATE VIRTUAL TABLE IF NOT EXISTS mail_fts USING fts5(message_row_id UNINDEXED,content,tokenize='unicode61');
CREATE TABLE IF NOT EXISTS calendars (id TEXT PRIMARY KEY,name TEXT NOT NULL,color TEXT,hex_color TEXT,owner_name TEXT,owner_address TEXT,is_default INTEGER NOT NULL DEFAULT 0,can_edit INTEGER NOT NULL DEFAULT 0,can_share INTEGER NOT NULL DEFAULT 0,can_view_private INTEGER NOT NULL DEFAULT 0,enabled INTEGER NOT NULL DEFAULT 1,updated_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS calendar_events (row_id INTEGER PRIMARY KEY AUTOINCREMENT,id TEXT NOT NULL,calendar_id TEXT NOT NULL,ical_uid TEXT,series_master_id TEXT,event_type TEXT,subject TEXT,body_html TEXT,body_text TEXT,body_preview TEXT,start_utc TEXT,end_utc TEXT,start_timezone TEXT,end_timezone TEXT,original_start_timezone TEXT,original_end_timezone TEXT,is_all_day INTEGER,is_cancelled INTEGER,is_online_meeting INTEGER,is_organizer INTEGER,is_draft INTEGER,has_attachments INTEGER,organizer_name TEXT,organizer_address TEXT,response TEXT,teams_join_url TEXT,web_url TEXT,sensitivity TEXT,show_as TEXT,importance TEXT,created_at TEXT,modified_at TEXT,deleted_at TEXT,raw_json TEXT NOT NULL,indexed_at TEXT,UNIQUE(calendar_id,id),FOREIGN KEY(calendar_id) REFERENCES calendars(id));
CREATE TABLE IF NOT EXISTS calendar_attendees (event_row_id INTEGER NOT NULL,attendee_type TEXT,name TEXT,address TEXT,response TEXT,FOREIGN KEY(event_row_id) REFERENCES calendar_events(row_id));
CREATE TABLE IF NOT EXISTS calendar_locations (event_row_id INTEGER NOT NULL,name TEXT,address TEXT,location_type TEXT,unique_id TEXT,unique_id_type TEXT,FOREIGN KEY(event_row_id) REFERENCES calendar_events(row_id));
...
CREATE INDEX IF NOT EXISTS messages_container_created ON messages(container_id,created_at); CREATE INDEX IF NOT EXISTS messages_sender ON messages(sender_id);`)
	...
	if _, e := s.DB.ExecContext(ctx, `ALTER TABLE mail_messages ADD COLUMN deleted_at TEXT`); e != nil && !strings.Contains(strings.ToLower(e.Error()), "duplicate column") {
		return e
	}
	return nil
}
