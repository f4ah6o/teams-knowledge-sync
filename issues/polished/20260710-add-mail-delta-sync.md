...
- [ ] フォルダーごとにdeltaリンクが独立して保存される。
- [ ] nextLinkの全ページを処理し、完了後だけdeltaLinkが更新される。
- [ ] 追加・更新・削除・移動をローカルへ反映できる。
- [ ] 同じdeltaページを再処理しても重複しない。
- [ ] 429、401/403、token失効、単一フォルダー失敗を記録し、他フォルダー同期を継続できる。
- [ ] Daemonの定期実行で対象フォルダーを再同期できる。
