...
以下の方針どおり実装した。Microsoft Graph v1.0のcalendarView deltaは既定予定表だけを対象とするため、非既定予定表を明示選択した場合のみbetaの`/me/calendars/{id}/calendarView/delta`を使用する。
...
calendarView deltaは開始・終了日時を初回クエリへ固定し、後続はtoken URLをそのまま再利用する必要がある。範囲外の削除通知が返るため、イベントとウィンドウの対応を保持し、全対応が削除された場合だけTombstone化する。非既定予定表はbeta endpointに依存するため、Microsoft Graphの仕様変更時に追随が必要である。
...
- 2026-07-10: Calendar CLI初回同期の完了を受け、ウィンドウ単位delta同期の実装へ着手。
- 2026-07-10: `go test -race ./...`、`go vet ./...`が成功した。ウィンドウ生成・境界・nextLink/deltaLink・追加/更新/削除・token失効完全照合・範囲外削除・429/403障害分離・未来延長・daemon反復を自動検証した。Microsoft 365実環境確認は資格情報が必要なため未実施。
- 2026-07-10: Calendar window delta同期、局所復旧、未来延長、障害分離の実装と検証が完了したため完了。
