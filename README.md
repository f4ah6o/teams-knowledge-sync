...
同じリポジトリの`outlook`バイナリで、OutlookメールとOutlook予定表をローカル SQLite（`./data/outlook-knowledge.db`）へ同期しCLIで検索できます。
...
3. `go run ./cmd/outlook auth login`、続いて`go run ./cmd/outlook mail sync`を実行します。
...
```
outlook auth login|status|logout
outlook mail address list
outlook mail folder list
outlook mail sync [--folder FOLDER_ID] [--full]
outlook mail search "工事引継" --address project-ml@example.com
outlook mail show MESSAGE_ID
outlook mail thread MESSAGE_ID
outlook mail status --json
outlook calendar list
outlook calendar sync [--calendar CALENDAR_ID] [--from 2026-07-01 --to 2026-07-31] [--full]
outlook calendar day 2026-07-10
outlook calendar range 2026-07-01 2026-07-31
outlook calendar search "定例"
outlook calendar show EVENT_ID
outlook calendar status --json
outlook daemon
```
...
Microsoft Graph v1.0のcalendarView deltaは既定予定表では`/me/calendarView/delta`を使い、明示選択した非既定予定表ではbetaの`/me/calendars/{id}/calendarView/delta`を使います。beta APIは仕様変更の可能性があるため、実運用ではMicrosoft Graphの現行仕様を確認してください。
